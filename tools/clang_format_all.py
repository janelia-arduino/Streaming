...
- PlatformIO / Arduino projects often contain `.ino` sources, which we also
  include.
- Project documentation may embed C/C++ examples in Org or Markdown files, and
  those snippets should follow the same `.clang-format` rules as the real code.

Usage:
  python tools/clang_format_all.py          # formats files in-place
  python tools/clang_format_all.py --check  # fails if formatting changes are needed

This script:
- finds files via `git ls-files` (so it only touches tracked files)
- formats source files with `clang-format --style=file`
- formats supported embedded code blocks inside tracked `.org` and `.md` files
